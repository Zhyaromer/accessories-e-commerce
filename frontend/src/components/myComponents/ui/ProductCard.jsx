...
const ProductCard = ({ product }) => {
  ...
  return (
    <div onClick={() => window.location.href = `/product/${product.id}`} className={`cursor-pointer rounded-lg overflow-hidden shadow-md ${Boolean(product.isSoldOut) ? 'bg-gray-200' : 'bg-white'} hover:shadow-lg transition-shadow duration-300 flex flex-col h-full relative`}>
      ...
      <div className="relative">
        <img 
          src='https://m.media-amazon.com/images/I/614rbIVRUJL._AC_SX569_.jpg' 
          alt={product.title} 
      ...
      <div className="p-2 md:px-3 md:py-2 flex-grow flex flex-col justify-between">
        <div>
          ...
          <div className="mt-1 md:mt-2 flex items-center">
            <span className="text-xs text-gray-600 mr-1 hidden sm:inline">Colors:</span>
        ...
        <div className="flex justify-between items-center mt-2 md:mt-3">
            ...
            {Boolean(product.isDiscounted) ? (
              <div className="flex items-center">
                <span className="font-bold text-xs md:text-sm text-red-600">
                  ${product.discount_price !== null ? product.discount_price.toFixed(2) : ''}
                </span>
          ...
          <button 
            className={`px-1 py-0.5 sm:px-2 sm:py-1 rounded-lg flex items-center justify-center cursor-pointer text-xs ${
